...
- **THEN** el frontend muestra un mensaje de error genérico sin revelar si el usuario existe o no

---

### Requirement: Servicio de email configurable por variables de entorno
El sistema SHALL disponer de un `EmailService` que encapsule el envío de emails via SMTP. La configuración SHALL leerse de variables de entorno: `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_USER`, `EMAIL_PASS`, `EMAIL_FROM`.
...
El sistema SHALL enviar un email con la nueva contraseña generada cuando se produce un reset.
